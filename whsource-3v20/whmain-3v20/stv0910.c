...
/*
	Note that inside the Serit NIM, tuner1 is connected to demodulator2 and vice-versa.
	It should be possible to change this using register values, but it does not currently work.
	To access the tuner1 output, demod2 P2 registers must be used and vice-versa.
	STV0910_DEMOD_TOP has been replaced by STV_TUNER1_DEMOD2 (=1) as either tuner can be connected to either antenna.
	Passed parameter 'demod' has been replaced by 'tunerdemod'. 
	If tunerdemod == STV_TUNER1_DEMOD2, the P2 demodulator2 registers are accessed to access tuner1 output.
*/
...
/*																									  */
/*   tunerdemod: 	the  tuner/demod combination (1 or 2)								           	  */
/*   sr: 			the symbol rate to initialise the demodulator (0 = disable)    					  */
/* 	 calibrated:	the rx has been calibrated for QO100, 											  */
/*  				so the frequency search width can be restricted                                   */
/* 					                                                                                  */
/* return: error code                                                                                 */
/* -------------------------------------------------------------------------------------------------- */

uint8_t stv0910_setup_receive (uint32_t tunerdemod, uint32_t sr, int32_t calibrated) 
{
    uint8_t 	err = ERROR_NONE ; 

    printf ("Flow: STV0910 init\r\n") ;

/* first we stop the demodulator in case it is already running */

///    if (err == ERROR_NONE) err = stv0910_write_reg (RSTV0910_P1_DMDISTATE, 0x1c) ;

    err = stv0910_write_reg 
    (
    	tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_DMDISTATE : RSTV0910_P1_DMDISTATE, 0x1c
	) ;

/* now we do the inits for the demodulator */
	
    if (err == ERROR_NONE) err = stv0910_setup_equalisers   (tunerdemod) ;
    if (err == ERROR_NONE) err = stv0910_setup_carrier_loop (tunerdemod, sr, calibrated) ;
    if (err == ERROR_NONE) err = stv0910_setup_timing_loop  (tunerdemod, sr) ;

    if (err != ERROR_NONE) printf ("ERROR: STV0910_setup_receive \r\n") ;
...
uint8_t stv0910_read_rolloff (uint8_t tunerdemod, uint32_t *rolloff)		
{
			uint8_t		err ;
			uint8_t		temp0 ;		
    ...
    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_TMGOBS : RSTV0910_P1_TMGOBS, &temp0) ; 
	*rolloff = temp0 >> 6 ;
...
uint8_t stv0910_read_multistream0 (uint8_t tunerdemod, uint32_t *info)		
{
			uint8_t		err ;
			uint8_t		temp0 ;		
			uint8_t		temp1 ;
			uint8_t		temp2 ;		
			uint8_t		temp3 ;
	static	uint8_t		modex ;		// cycles around 0,1,2 to get current,lowest,highest stream number

	err = ERROR_NONE ;

    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_MATSTR0   : RSTV0910_P1_MATSTR0,   &temp0) ; 
    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_MATSTR1   : RSTV0910_P1_MATSTR1,   &temp1) ; 
    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_PDELCTRL0 : RSTV0910_P1_PDELCTRL0, &temp2) ; 
    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_PDELCTRL1 : RSTV0910_P1_PDELCTRL1, &temp3) ; 

	*info = temp0 | (temp1 << 8) | (temp2 << 16) | (temp3 << 24) ;

	modex++ ;						// step the MATYPE data to the next type
	if (modex > 2)
	{
		modex = 0 ;
	}
    err |= stv0910_write_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_PDELCTRL0 : RSTV0910_P1_PDELCTRL0, modex << 4) ; 	

    if (err != ERROR_NONE) printf ("ERROR: STV0910 read multistream0\r\n") ;
...
uint8_t stv0910_read_multistream1 (uint8_t tunerdemod, uint32_t *info)		// (!~!~!) 					
{
			uint8_t		err ;
			uint8_t		temp0 ;		
			uint8_t		temp1 ;
    ...
    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_ISIENTRY  : RSTV0910_P1_ISIENTRY,  &temp0) ; 
    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_ISIBITENA : RSTV0910_P1_ISIBITENA, &temp1) ; 

	*info = temp0 | (temp1 << 8) ;
    if (err != ERROR_NONE) printf ("ERROR: STV0910 read multistream1\r\n") ;
...
uint8_t stv0910_read_debug0 (uint8_t tunerdemod, uint32_t *info)		// (!~!~!) 					
{
			uint8_t		err ;
			uint8_t		temp0 ;		
			uint8_t		temp1 ;
    ...
    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_TSSTATUS : RSTV0910_P1_TSSTATUS,  &temp0) ; 
    err |= stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_TSSTATUS2: RSTV0910_P1_TSSTATUS2, &temp1) ; 
...
uint8_t stv0910_read_matstr1 (uint8_t tunerdemod, uint8_t *info)		 					
{
	uint8_t		err ;

    err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_MATSTR1 : RSTV0910_P1_MATSTR1, info) ; 
...
uint8_t stv0910_read_pdelctrl1 (uint8_t tunerdemod, uint8_t *info)		
{
	uint8_t		err ;

    err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_PDELCTRL1 : RSTV0910_P1_PDELCTRL1, info) ; 
...
uint8_t stv0910_write_pdelctrl1 (uint8_t tunerdemod, uint8_t info)						
{
	uint8_t		err ;

    err = stv0910_write_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_PDELCTRL1 : RSTV0910_P1_PDELCTRL1, info) ; 
...
uint8_t stv0910_write_isientry (uint8_t tunerdemod, uint8_t info)		 					
{
	uint8_t		err ;

    err = stv0910_write_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_ISIENTRY : RSTV0910_P1_ISIENTRY, info) ; 
...
uint8_t stv0910_write_isibitena (uint8_t tunerdemod, uint8_t info)						
{
	uint8_t		err ;

    err = stv0910_write_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_ISIBITENA : RSTV0910_P1_ISIBITENA, info) ; 
...
/*    tunerdemod: STV_TUNER1_DEMOD2 | STV0910_DEMOD_BOTTOM: which demodulator is being read            */
/* car_freq: signed place to store the answer                                                         */
/*   return: error state                                                                              */
/* -------------------------------------------------------------------------------------------------- */

uint8_t stv0910_read_car_freq(uint8_t tunerdemod, int32_t *cf) 
{
    uint8_t 	err = ERROR_NONE ;
...
/* first off we read in the carrier offset as a signed number */
    ...
    err = stv0910_read_reg(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_CFR2 : RSTV0910_P1_CFR2, &val_h) ; 		/* high byte*/
    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_CFR1 : RSTV0910_P1_CFR1, &val_m) ; 	/* mid */
	}
	
    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_CFR0 : RSTV0910_P1_CFR0, &val_l) ; 	/* low */
	}
...
/*   demod: STV_TUNER1_DEMOD2 | STV0910_DEMOD_BOTTOM: which demodulator is being read                 */
/*  return: error state                                                                               */
/* -------------------------------------------------------------------------------------------------- */

uint8_t stv0910_read_constellation(uint8_t tunerdemod, uint8_t *i, uint8_t *q) {

    uint8_t 	err = ERROR_NONE ;

    err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_ISYMB : RSTV0910_P1_ISYMB, i) ;
    
	if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_QSYMB : RSTV0910_P1_QSYMB, q) ;
	}
...
uint8_t stv0910_read_sr (uint8_t tunerdemod, uint32_t *found_sr) 
{
	double 		sr ;
    ...
    err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_SFR3 : RSTV0910_P1_SFR3, &val_h) ;  		/* high byte */
    
	if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_SFR2 : RSTV0910_P1_SFR2, &val_mu); 	/* mid upper */
	}
    
	if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_SFR1 : RSTV0910_P1_SFR1, &val_ml); 	/* mid lower */
	}
	
    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_SFR0 : RSTV0910_P1_SFR0, &val_l);  	/* low byte */
    }
...
/* sr (MHz) = ckadc (MHz) * SFR/2^32. So in Symbols per Second we need: */
    ...
    sr = sr * 135000000 / 256.0 / 256.0 / 256.0 / 256.0 ;
...
// read the symbol rate detection offset
    ...
    if (err == ERROR_NONE) 
	{
	    ...
	    (
	    	tunerdemod == STV_TUNER1_DEMOD2 
	    	? RSTV0910_P2_TMGREG2 : RSTV0910_P1_TMGREG2, 		    /* byte2 */
	    	&tempc
	    ) ;  		
		temp |= tempc << 24 ;
	    ...
	    (
	    	tunerdemod == STV_TUNER1_DEMOD2 
	    	? RSTV0910_P2_TMGREG1 : RSTV0910_P1_TMGREG1, 		    /* byte1 */
	    	&tempc
	    ) ;  		
		temp |= tempc << 16 ;
	    ...
	    (
	    	tunerdemod == STV_TUNER1_DEMOD2 
	    	? RSTV0910_P2_TMGREG0 : RSTV0910_P1_TMGREG0, 		    /* byte0 */
	    	&tempc
	    ) ;  		
		temp |= tempc << 8 ;
...
/* -------------------------------------------------------------------------------------------------- */
/* reads the detected viterbi punctuation rate                                                        */
/*   demod: STV_TUNER1_DEMOD2 | STV0910_DEMOD_BOTTOM: which demodulator is being read                 */
...
uint8_t stv0910_read_puncture_rate (uint8_t tunerdemod, uint8_t *rate) 

{
    uint8_t 	err ;
    uint8_t 	val ;

    err = stv0910_read_reg_field (tunerdemod == STV_TUNER1_DEMOD2 ? FSTV0910_P2_VIT_CURPUN : FSTV0910_P1_VIT_CURPUN, &val) ;
    switch (val) 
	{
...
uint8_t stv0910_read_power (uint8_t tunerdemod, uint8_t *power_i, uint8_t *power_q) 
{
    uint8_t 	err;

/* power = 1/4 . ADC */

	err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_POWERI : RSTV0910_P1_POWERI, power_i) ;

	if (err == ERROR_NONE)
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_POWERQ : RSTV0910_P1_POWERQ, power_q) ;
	}
...
uint8_t stv0910_read_err_rate (uint8_t tunerdemod, uint32_t *vit_errs) 
{
    uint8_t 	err ;
    uint8_t 	val ;

    err = stv0910_read_reg(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_VERROR : RSTV0910_P1_VERROR, &val);
...
uint8_t stv0910_read_ber (uint8_t tunerdemod, uint32_t *ber) 
{
    uint8_t 	err ;
...
/* first we trigger a buffer transfer and read the byte counter 40 bits */
    ...
    err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_FBERCPT4 : RSTV0910_P1_FBERCPT4, &high) ;

    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_FBERCPT3 : RSTV0910_P1_FBERCPT3, &mid_u) ;
    }
	
	if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_FBERCPT2 : RSTV0910_P1_FBERCPT2, &mid_m) ;
	}
	
    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_FBERCPT1 : RSTV0910_P1_FBERCPT1, &mid_l) ;
	}
	
    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_FBERCPT0 : RSTV0910_P1_FBERCPT0, &low) ;
	}
	
    cpt = 	(double) high  * 256.0 * 256.0 * 256.0 * 256.0 + (double) mid_u * 256.0 * 256.0 * 256.0 + 
			(double) mid_m * 256.0 * 256.0 + (double) mid_l * 256.0 + (double)low;

/* we have already triggered the register buffer transfer, so now we we read the bit error from them */

    err = stv0910_read_reg(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_FBERERR2 : RSTV0910_P1_FBERERR2, &high) ;
	
    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_FBERERR1 : RSTV0910_P1_FBERERR1, &mid_m) ;
	}
	
    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_FBERERR0 : RSTV0910_P1_FBERERR0, &low) ;
	}
...
uint8_t stv0910_read_mer (uint8_t tunerdemod, int32_t *mer) 
{
    uint8_t 	err ;
    uint8_t		high, low ;
	int32_t		temp ;

    err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_NOSRAMPOS : RSTV0910_P1_NOSRAMPOS, &high) ;
    
	if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_NOSRAMVAL : RSTV0910_P1_NOSRAMVAL, &low) ;
    }

    if (((high >> 2) & 0x01) == 1)					/* Px_NOSRAM_CNRVAL is valid */
	{
		temp = ((high & 0x03) << 8) | low ;			// 10 bit signed value
		if (temp & 0x200)							// negative
		{
			temp |= 0xfffffc00 ;			
		}
		*mer = temp ;
    ...
    else
    {
        *mer = -300 ;
        if (err == ERROR_NONE) 
		{
			err = stv0910_write_reg_field (tunerdemod == STV_TUNER1_DEMOD2 ? FSTV0910_P2_NOSRAM_ACTIVATION : FSTV0910_P1_NOSRAM_ACTIVATION, 0x02) ;
		}
	}
...
uint8_t stv0910_read_errors_bch_uncorrected (uint8_t tunerdemod, bool *errors_bch_uncorrected) 
{
    uint8_t 	err ;
    uint8_t 	result ;
...
uint8_t stv0910_read_errors_bch_count (uint8_t tunerdemod, uint32_t *errors_bch_count) 
{
    uint8_t 	err ;
    uint8_t 	result ;
...
uint8_t stv0910_read_errors_ldpc_count (uint8_t tunerdemod, uint32_t *errors_ldpc_count) 
{
    uint8_t	 	err ;
    uint8_t 	high, low ;
...
uint8_t stv0910_read_modcod_and_type (uint8_t tunerdemod, uint32_t *modcod, bool *short_frame, bool *pilots) 
{
    uint8_t 	err ;
    uint8_t 	regval ;
    
    err = stv0910_read_reg (tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_DMDMODCOD : RSTV0910_P1_DMDMODCOD, &regval);
...
uint8_t stv0910_setup_equalisers (uint8_t tunerdemod) 
{
    printf ("Flow: Setup equalisers %i\n", tunerdemod) ;
...
uint8_t stv0910_setup_carrier_loop (uint8_t tunerdemod, uint32_t sr, uint32_t calibrated) 
{
    uint8_t 	err;
	int64_t		temp ;
	uint8_t		tempc ;
...
/* start at 0 offset */
	...
	err = stv0910_write_reg ((tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_CFRINIT0 : RSTV0910_P1_CFRINIT0), 0) ;
	
	if (err == ERROR_NONE) 
	{
		err = stv0910_write_reg((tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_CFRINIT1 : RSTV0910_P1_CFRINIT1), 0) ;
	}

// (WH) if calibrated is set, set the upper and lower frequency search bounds to SR / 2
	...
	{
		temp = 1000 ;										// default to +/- 1MHz
	}
	temp = temp * 65536 / 135000 ;							// work out the register values
	...
	if (err == ERROR_NONE) 
	{
		...
		(
			tunerdemod == STV_TUNER1_DEMOD2 
			? RSTV0910_P2_CFRUP0 : RSTV0910_P1_CFRUP0, 		// high side frequency offset limit
			(uint8_t) (temp & 0xff)
		) ;
		err = stv0910_write_reg
		(
			tunerdemod == STV_TUNER1_DEMOD2 
			? RSTV0910_P2_CFRUP1 : RSTV0910_P1_CFRUP1, 		
			(uint8_t) ((temp >> 8) & 0xff)
		) ;
	}
...
// the lower value is the negative of the upper value
	...
	if (err == ERROR_NONE) 
	{
		...
		(
			tunerdemod == STV_TUNER1_DEMOD2 
			? RSTV0910_P2_CFRLOW0 : RSTV0910_P1_CFRLOW0,	// low side frequency offset limit 	
			(uint8_t) (temp & 0xff)
		) ;
		err = stv0910_write_reg
		(
			tunerdemod == STV_TUNER1_DEMOD2 
			? RSTV0910_P2_CFRLOW1 : RSTV0910_P1_CFRLOW1, 
			(uint8_t) ((temp >> 8) & 0xff)
		) ;
	}

// turn off DVB-S detection for SR < 1000
	...
	if (err == ERROR_NONE) 
	{
		...
		(
			tunerdemod == STV_TUNER1_DEMOD2 
			? RSTV0910_P2_DMDCFGMD : RSTV0910_P1_DMDCFGMD, 	
			&tempc
		) ;
...
uint8_t stv0910_setup_timing_loop (uint8_t tunerdemod, uint32_t sr) 
{
    uint8_t 	err	= ERROR_NONE ;
    uint16_t 	sr_reg ; 
...
*/
    ...
    if (err == ERROR_NONE) 
	{
		...
		(
			tunerdemod == STV_TUNER1_DEMOD2 
			? RSTV0910_P2_SFRINIT1 : RSTV0910_P1_SFRINIT1,
			(uint8_t)(sr_reg >> 8) 
		) ;
	}
	...
	if (err == ERROR_NONE) 
	{
		...
		(
			tunerdemod == STV_TUNER1_DEMOD2 
			? RSTV0910_P2_SFRINIT0 : RSTV0910_P1_SFRINIT0, 
			(uint8_t)(sr_reg & 0xFF)
		) ;
	}	
...
uint8_t stv0910_setup_ts (uint8_t tunerdemod) 
{
    uint8_t 	err	= ERROR_NONE ;
...
uint8_t stv0910_start_scan (uint8_t tunerdemod) 
{
    uint8_t 	err = ERROR_NONE ;
    ...
    if (err == ERROR_NONE) 
	{
		err = stv0910_write_reg
		(
			(tunerdemod == STV_TUNER1_DEMOD2 ? RSTV0910_P2_DMDISTATE : RSTV0910_P1_DMDISTATE),
...
uint8_t stv0910_read_scan_state (uint8_t tunerdemod, uint8_t *state) 
{
    ...
    if (err == ERROR_NONE) 
	{
		err = stv0910_read_reg_field 
		(
			(tunerdemod == STV_TUNER1_DEMOD2 ? FSTV0910_P2_HEADER_MODE : FSTV0910_P1_HEADER_MODE), state
		) ;
	}
