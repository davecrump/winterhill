...
/*    - the LNA support routines (STVVGLNA). The Serit FT4334L NIM has 2 LNAs, the 4335 has none      */
...
/* first we decide which LNA to use */
    ...
    if (input == NIM_INPUT_TOP) 
    {
    	lna_addr = NIM_LNA_0_ADDR ;
    }
    else 
    {
    	lna_addr = NIM_LNA_1_ADDR ;
    }
...
/* now check to see if there is an LNA present */
    ...
    else 											 	/* otherwise, lna is there so we go on to us it */
    {
...
/* set up the defaults. We are going to use fully automatic mode */
        ...
        {        
            if (err == ERROR_NONE)	
            {
            	err = stvvglna_write_reg
            	(
            	 	lna_addr, STVVGLNA_REG0,
...
/* variable part of the gain for us. Note, it is ok to write 0 to the other bitfields */
    ...
    if (err==ERROR_NONE) 
    {
    	err=stvvglna_write_reg(lna_addr, STVVGLNA_REG1, STVVGLNA_REG1_GETAGC_START << STVVGLNA_REG1_GETAGC_SHIFT) ;
	}
    ...
    do 
    {
        ...
        if ((err == ERROR_NONE) && (timeout == STVVGLNA_AGC_TIMEOUT)) 
        {
            ...
            printf ("Error: reading AGC timeout\r\n");
        }
    }
    ...
    if (err == ERROR_NONE) 
    {
    	err=stvvglna_read_reg(lna_addr, STVVGLNA_REG3, gain);  /* read out the gain curves */
	}
    ...
    *gain = ((*gain & STVVGLNA_REG3_SWLNAGAIN_MASK) >> STVVGLNA_REG3_SWLNAGAIN_SHIFT) ;
    ...
    if (err == ERROR_NONE) 
    {
    	err=stvvglna_read_reg(lna_addr, STVVGLNA_REG1, vgo) ; /* read out the Vagc value */
    }
    ...
    *vgo = ((*vgo & STVVGLNA_REG1_VGO_MASK) >> STVVGLNA_REG1_VGO_SHIFT) ;
    ...
    if (err != ERROR_NONE) 
    {
    	printf ("ERROR: Failed LNA aquire AGC %i\n", input) ;
    }
    return err ;
}
...
*/
    ...
    printf ("LNA 0x%.2x regs: ", addr) ;
    for (i = 0 ; i < 4 ; i++) 
    {
       stvvglna_read_reg (addr, i, &val) ;
       printf ("0x%02X=0x%02X ", i, val) ;
    }
    printf ("\r\n") ;
}
